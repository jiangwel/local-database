...
void MixTest1Call() {
  for (size_t iter = 0; iter < NUM_ITERS; iter++) {
    ...
    // there is a bug: some keys are not deleted
    for (auto itr = tree.Begin(); !itr.IsEnd() && size != for_insert.size(); ++itr) {
      auto pair = *itr;
 ...
 */
TEST(BPlusTreeConcurrentTest3, InsertTest1) {
  TEST_TIMEOUT_BEGIN
  InsertTest1Call();
  remove("test.db");
  remove("test.log");
 ...
 * without overlap.
 */
TEST(BPlusTreeConcurrentTest3, InsertTest2) {
  TEST_TIMEOUT_BEGIN
  InsertTest2Call();
  remove("test.db");
  remove("test.log");
 ...
 */
TEST(BPlusTreeConcurrentTest3, DeleteTest1) {
  TEST_TIMEOUT_BEGIN
  DeleteTest1Call();
  remove("test.db");
  remove("test.log");
 ...
 * without overlap.
 */
TEST(BPlusTreeConcurrentTest3, DeleteTest2) {
  TEST_TIMEOUT_BEGIN
  DeleteTest2Call();
  remove("test.db");
  remove("test.log");
 ...
 */
TEST(BPlusTreeConcurrentTest3, DISABLED_MixTest1) {
  TEST_TIMEOUT_BEGIN
  MixTest1Call();
  remove("test.db");
  remove("test.log");
 ...
 * inserted.
 */
TEST(BPlusTreeConcurrentTest3, DISABLED_MixTest2) {
  TEST_TIMEOUT_BEGIN
  MixTest2Call();
  remove("test.db");
  remove("test.log");
 ...
 * deleted and new keys are added correctly.
 */
TEST(BPlusTreeConcurrentTest3, DISABLED_MixTest3) {
  TEST_TIMEOUT_BEGIN
  MixTest3Call();
  remove("test.db");
  remove("test.log");
